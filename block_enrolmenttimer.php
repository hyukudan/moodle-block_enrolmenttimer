...
class block_enrolmenttimer extends block_base {
    ...
    /** @var string Completion percentage config. */
    public $completionpercentage;

    /** @var int Active countdown config. */
    public $activecountdown;

    /** @var string View options (comma-separated unit IDs). */
    public $viewoptions;
    ...
    public function get_content() {
        ...
        if (!$timeleft) {
            ...
            if ($displaynothingnodateset == 1) {
                ...
                $this->content->text .= '<p class="noDateSet">' .
                    get_string('noDateSet', 'block_enrolmenttimer') . '</p></div>';
                return $this->content;
            }
        } else {
            $counter = 1;
            ...
            foreach ($timeleft as $unit => $count) {
                ...
                if ($countlength == 1 && $force2digits == 1) {
                    ...
                    for ($i = 0; $i < $countlength; $i++) {
                        $this->content->text .= '<span class="timerNumChar" data-id="' . $i . '">' .
                            $stringcount[$i] . '</span>';
                    }
                }
                ...
                if ($displaylabels == 1) {
                    $this->content->text .= '<p>' . $safeunit . '</p></div>';
                }
